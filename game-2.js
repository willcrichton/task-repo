// TODO: fix game when blocks at top.
...
const moveShape = () => {
  if (falling) {
    fs.forEach((f) =>
      new Square(30 * (fsCol + f[0]), 30 * (fsRow + f[1]), 30).draw(gt, col)
    );
...
function removeLine(line) {
  for (var c = 0; c < nc; c++) grid.squares[line][c] = -1;

  for (var c = 0; c < nc; c++) {
    for (var r = line; r > 0; r--) grid.squares[r][c] = grid.squares[r - 1][c];
  }
}
